"""
Recurrent IPPO for SocialJax single-environment variants.

This follows the scanned-GRU PPO structure used in the JaxMARL
overcookedv2 single-agent trainer, adapted to SocialJax observations with
shape (num_envs, num_agents, H, W, C).
"""
import functools
import os
import pickle
...
def batchify_reward(x, num_actors):
    return jnp.asarray(x).reshape((num_actors,))


def batchify_done(done: dict, agent_list, num_actors):
    return jnp.stack([done[str(a)] for a in agent_list]).reshape((num_actors,))
...
def make_train(config):
    ...
    if not config.get("PARAMETER_SHARING", True):
        raise NotImplementedError("The recurrent single-env IPPO trainer currently supports PARAMETER_SHARING=True.")
        ...
        def _update_step(runner_state, unused):
            def _env_step(runner_state, unused):
                ...
                transition = Transition(
                    ...
                    action,
                    value,
                    batchify_reward(reward, config["NUM_ACTORS"]),
                    log_prob,
                    obs_batch,
                    info,
                )
...
def evaluate(params, env, config):
    ...
    for o_t in range(config["GIF_NUM_FRAMES"]):
        ...
        done_batch = jnp.array([done[str(a)] for a in env.agents])
    ...
    gif_path = root_dir / (
        f"{env.num_agents}-agents_seed-{config['SEED']}_frames-{config['GIF_NUM_FRAMES']}.gif"
    )
    pics[0].save(
        gif_path,
        format="GIF",
    ...
    wandb.log({"Episode GIF": wandb.Video(str(gif_path), caption="Evaluation Episode", format="gif")})


def single_run(config, run_name):
    ...
    filename = f'{config["ENV_NAME"]}_rnn_seed{config["SEED"]}'
...
def tune(default_config, run_name):
    import copy
    ...
    def wrapped_make_train():
        ...
        wandb.run.name = f"sweep_{config['ENV_NAME']}_rnn_seed{config['SEED']}"
        rng = jax.random.PRNGKey(config["SEED"])
        rngs = jax.random.split(rng, config["NUM_SEEDS"])
...
def main_from_config(config, run_name):
    if config["TUNE"]:
