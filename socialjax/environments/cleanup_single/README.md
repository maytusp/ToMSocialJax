...
Clean Up Single is a one-player version of Clean Up where the agent earns +1 reward by collecting apples. Apples grow in an orchard and their regrowth depends on the cleanliness of a nearby river. Pollution accumulates in the river over time and once pollution surpasses a threshold, the apple growth rate drops. The agent can perform a cleaning action that removes small amounts of pollution from the river, so continuous apple growth depends on balancing foraging with cleaning.
...
for o_t in range(num_outer_steps):
    ...
    for t in range(num_inner_steps):
        ...
        actions = [jax.random.choice(
            ...
            p=jnp.array([0.1, 0.1, 0.09, 0.09, 0.09, 0.19, 0.05, 0.29])
